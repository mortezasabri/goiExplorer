...
pipeline <- function(dds,
  ...
  rm(g); g <- ggpubr::ggboxplot(df, x = intgroup, y = "count", 
                                fill = intgroup, 
                                palette = palette) +
    ggplot2::geom_point(position = ggplot2::position_dodge(width=0.75), 
                        ggplot2::aes(group=condition), 
                        color = "steelblue4") + 
    ggpubr::stat_pvalue_manual(stat.test.max, label = "padj: {p} {p.signif}",
  ...
  ## ----Count Plot
  rm(g); g <- ggplot2::ggplot(geneCounts, 
                              ggplot2::aes(x = condition, y = count, color = condition)) +
    ggplot2::scale_y_log10() + ggbeeswarm::geom_beeswarm(cex = 3) + 
    ggplot2::ggtitle(goi) +
  ...
  rm(g); g <- ggplot2::ggplot(res.df) +
    ggplot2::geom_point(ggplot2::aes(x = log2FoldChange, y = -log10(padj), colour = threshold), 
                        size =0.4) +
    ggrepel::geom_text_repel(aes(x = log2FoldChange, y = -log10(padj), 
